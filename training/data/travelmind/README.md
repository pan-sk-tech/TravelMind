...
legacy 数据只作为历史反例库和 pipeline baseline，不进入 travelmind 主训练集。

## 当前状态

- 当前冻结评估输入为 `eval/records.jsonl` 和 `eval_hard/records.jsonl`。
- 旧名 `eval_harder` 不再作为主入口。
- travelmind SFT 造数脚本主入口是 `training/scripts/travelmind/data/generate_sft_data.py`。
- 2026-05-08 之前的旧 TravelMind SFT 已归档；不要把新 realbudget 数据混写回旧 `sft/`，新 run 使用 `sft_runs/`。
