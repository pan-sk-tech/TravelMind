...
| `data/travelmind/eval/` | 冻结输入 | standard 评估集，公开保留 `records.jsonl`、`requests.jsonl`、摘要和重建说明。 |
| `data/travelmind/eval_hard/` | 冻结输入 | hard 评估集。旧名 `eval_harder` 不再作为主入口。 |
| `data/travelmind/attraction_prices/` | 票价资产 | `reports/` 放审核说明，`snapshots/` 放小体积票价表快照，`generated/` 放候选/估价 JSONL 和日志。 |
| `data/travelmind/sft*/` | 本地生成 | SFT run、realbudget run、best-of-n 数据默认按明确 run 目录生成并忽略，不能混写到旧目录。 |
| `data/travelmind/dpo/` | 本地生成 | prompt/candidate/pair/judge 数据默认本地保留；公开文档只描述口径。 |
...
| `scripts/serving/` | 服务入口 | 本地 TravelMind 模型服务和服务管理脚本。 |
| `scripts/validation/` | 校验入口 | SFT/DPO/Eval 输出 schema 与格式校验。 |
| `scripts/travelmind/` | 当前主线 | 按 `data/`、`eval/`、`audit/`、`pricing/`、`bestofn/`、`training/` 分组。 |
...
- 新 SFT/DPO 数据必须写入明确 run 目录，并带 manifest 或摘要；不要复用旧 `data/travelmind/sft/` 作为长期混合池。
- 评测输入只使用 `data/travelmind/eval/` 和 `data/travelmind/eval_hard/`。如果后端检索逻辑变了，先 rebuild context，保留同一批 request 和 record id。
