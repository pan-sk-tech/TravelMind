...
所以这篇教程会按真实推进顺序写：先改前后端协议，再固定评测集，接着调 prompt 找到边界，然后用强模型生成 SFT 数据并做审计；SFT 学会基本协议后，再用 DPO 学偏好，最后用多候选 rerank 收尾。
...
到最后，项目不是靠一个神奇 checkpoint 收尾，而是靠一套分层判断：Prompt 固定协议，SFT 学会结构，DPO 学偏好，Rerank 在候选里选更稳的答案。下面这张图先给一个全局观感，细节后面慢慢拆。不要把它读成严格同口径排行榜，它更像实验路线图。

![后训练阶段指标变化图](./images/旅行助手后训练实战/08-stage-metrics-trend.png)

## 开篇 Case：先看模型到底变好了什么

先放一条普通 case。不是为了证明模型有多强，只是想让你先有个手感：后训练前后，到底差在哪。
...
如果只看这张表，已经能看出后训练的收益不是“文案更漂亮”，而是更接近产品协议：住宿天数、预算关系、餐饮重复、行程密度这些东西开始稳定了。
...
最终版也不是神奇地变成满分，但它明显更像一个能交给用户的计划：前三晚酒店稳定在同一家 250 元经济型酒店；景点从一天一个变成了西湖、断桥、雷峰塔、灵隐寺、植物园、西溪、德寿宫、城市阳台、清河坊这样的组合；餐厅也开始轮换，有杭帮菜、海鲜、烤肉、湿地附近餐厅、面馆和快餐。预算报到 2500 元，刚好落在这类 limited hard case 的可接受区间下沿。

当前规则评测还能抓到一个小账问题：返回里 `budget.total_meals` 是 1020 元，规则重算是 960 元，相差 60 元。所以这个例子想说明的不是“训练后就完美了”，而是后训练最真实的收益：先把用户一眼能看出来的错压下去，再用固定指标继续追剩下的细账。至于更细的 POI 字段一致性，也可以在后续评测里继续加规则。

后面讲的 hardpass、softpass、预算关系和切片评测，本质上都在做同一件事：把这种肉眼能看出来的差别，变成一批 case 上可以稳定比较的数字。
...
这套教程最好先把环境配好再往下做。旅行助手后训练不是只跑一个 notebook，它会同时用到前后端协议、地图候选、强模型数据生成、规则评测和 LoRA 训练。环境没理顺的话，后面每一步都会变成排错。
...
这份数据适合和本文里的阶段说明一起看：前期可以先对照评测报告和样例输出，后面真正复现 LoRA 时，再把本地路径替换成自己下载后的数据路径。实际训练时仍然建议保留每一轮的 `manifest`、usage 和审计报告，因为这些文件能说明数据从哪来、过滤掉了什么、最后进训练集的又是哪一批。

如果网盘数据和仓库里的 `training/docs/后训练产物` 有细节差异，以教程里对应阶段的说明和本地 `manifest` 为准。网盘更像是给读者快速上手的材料包，不是替代实验记录本身。

### 0.3 大概要多少资源

这件事可以分档做，不是每一步都要 4 张 GPU。
...
我的建议是先按 `smoke 20 -> 审计 -> 100 条 -> 审计 -> 1000 条 -> LoRA` 的顺序推进。资源少的时候不要急着缩指标口径，先缩样本量和训练轮数；否则最后跑通了，也不知道结果还能不能和主线对上。
...
SFT 到这里已经能把 TripPlan 的壳子写稳了，但还有一个很真实的问题：合法答案之间也有好坏。两个计划都能过 schema，都能找到酒店和餐厅，一个可能很省但不像用户想要的旅行，另一个预算更贴合、餐饮更少重复、景点也更顺。SFT 很难从单条 teacher 里稳定学到这种取舍，DPO 更适合做这件事。

我这里没有把 DPO 当成“万能增强”。它只做一件事：在 hardpass 已经过关的候选里，学习哪个更像一个好行程。
...
还有一条底线：不能从冻结评测集里挖训练 pair。后面预算收尾数据里专门做了签名过滤，`selected_eval_signature_overlap = 0`。这件事很烦，但必须做。不然分数看起来好，实际上是在背题。
...
有一轮预算收尾训练的 loss 看起来比前几轮大很多。第一眼会紧张，但这里不能直接横比。前几轮 pair 很容易分，loss 低、accuracy 高；预算收尾 pair 更接近，chosen 和 rejected 都是 hardpass 计划，只是在预算使用、重复和偏好上有差别，loss 自然会更高。
...
DPO 收尾最有意思的地方在这里：`260519 ckpt138` 是单生成最稳的点，但最终展示版本是 `ckpt64_rerank_n4`。这听起来有点绕，其实很合理。单生成看的是一次采样的平均质量；rerank 看的是候选池里有没有更好的答案，以及规则能不能把它选出来。
...
这张图最该看的不是某个点的绝对值，而是 soft / planner soft 一路往上走，hardpass 没被明显牺牲。预算相关指标也在 rerank 后有明显回升。
...
把 SFT 主线都画进去以后，两个信号会更清楚：前半段继续 SFT 的收益已经变钝，后半段接入 rerank 后才真正把 softpass 和重算预算 softpass 往上推。
...
DPO 后半段最容易误读。单看单生成，`260519 ckpt138` 更稳；继续做预算收尾训练后，`ckpt66` 和 `ckpt64` 没有把单生成分数继续推高。
...
把 DPO 主线从 SFT final 起点一路画到最终 rerank，会更直观：

![DPO 收尾 Rerank 效果图](./images/旅行助手后训练实战/10-dpo-rerank-closing.png)

这里的结论不是“最后一炉单生成最好”。`260519 ckpt138` 仍然是单生成最稳的点；预算收尾训练更像是在改变候选分布，配合多温度 rerank 后，`ckpt64_rerank_n4` 才在 planner soft 和重算预算 soft 上明显跳起来。
...
所以公开写法不要写成“全面超过 MiMo”。我会写成：在本项目冻结评测和规则口径下，最终本地模型的 planner soft 已经追平强模型参考；预算贴合仍有差距，后续如果继续做，应该补预算总额控制和预算档位之间的协调。
