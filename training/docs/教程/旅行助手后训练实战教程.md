...
先放一个同一条评测 case 的前后对比。后面会讲 hardpass、softpass、预算关系和切片指标，但这些词第一次看会有点抽象。看完这个例子，再回头看指标会更容易知道它们到底在管什么。

这条请求很普通：

```text
杭州 2026-05-11 到 2026-05-14，单人自由行，打车，经济型酒店。
偏好是美食和城市地标，硬预算 3500 元，节奏适中。
```

| 版本 | 结果长什么样 |
| --- | --- |
| 基础模型 | 能写出西湖、灵隐寺、城市阳台这些景点，但预算和行程口径明显不稳。规则评测报了 9 类错误：住宿类型不一致、同一天午晚餐重复、餐厅重复太多、酒店预算低估、预算加总不一致、景点预算不一致、餐饮预算不一致、预算关系不一致、预算档位偏好不一致。它还把最后一天写成“无住宿”，餐厅里反复出现肯德基，整趟预算只报了 1840 元，和 3500 元硬预算下的 limited 档位不太匹配。 |
| 最终版 LoRA + 多温度 rerank | 同一条请求下，最终版只剩一个餐饮小账误差：餐费明细重算和 `budget.total_meals` 还有差距。住宿稳定成 250 元左右一晚的经济型酒店，餐厅从知味观、杭帮菜、海鲜排档、烤肉、西溪餐厅到面馆都有轮换；景点也从每天一个点扩展到西湖、断桥、雷峰塔、灵隐寺、植物园、西溪、德寿宫、城市阳台、清河坊。预算报到 2500 元，仍在 3500 元硬预算内，也比基础版更接近预算档位。 |

这个 case 不是为了证明最终版“满分”。它恰好说明了后训练最常见的真实状态：体验上已经从“像模板生成”变成“可以拿来看的计划”，但评测还能继续抓到细账问题。后面所有指标，本质上都是在把这种肉眼能看到的差别批量化。
