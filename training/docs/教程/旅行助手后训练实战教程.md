...
先别急着讲 hardpass、softpass，也别急着把预算指标搬出来。读者刚打开一篇后训练教程时，最需要的不是一屏指标，而是先建立一个直觉：同一条请求，训练前后到底哪里不一样。

我建议开头就放一条固定评测集里的真实 case。但不要直接把两份 JSON 拍到读者脸上。更顺的读法是：

1. 先用一张小表看结论。
2. 再看几个肉眼能发现的问题。
3. 最后把完整原始返回折叠起来，方便想核对细节的人展开。

这条 case 的用户请求是：一个人去杭州玩 4 天，打车，住经济型酒店，偏好美食和城市地标，总预算 3500 元左右，预算是 hard constraint。
