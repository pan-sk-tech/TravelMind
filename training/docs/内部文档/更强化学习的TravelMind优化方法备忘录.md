# 更强化学习的 TravelMind 优化方法备忘录

## 当前决策

短期先继续推进 DPO。

原因是当前 `travelmind_soft_pass` 已经被证明可以通过 DPO 学到，且本轮 direct402 checkpoint sweep 里 `ckpt-25` 已经把 combined travelmind soft 从 SFT final 的 35.2% 拉到 44.7%。下一步最稳的是围绕 travelmind soft 继续补高质量 pair，而不是马上切到更复杂的在线强化学习训练。

后续如果要尝试更接近严格强化学习的方法，可以把下面几种方案作为备选。

## 当前任务为什么适合做 RL-style 实验

TravelMind 任务本质上不是单纯语言风格对齐，而是一个带约束的组合决策问题：

- 输入是一个固定的 `TravelMindContext` / 用户请求。
...
```text
state   = TravelMindContext / prompt
action  = generated TripPlan
reward  = rule evaluator score
policy  = travelmind model
```
...
- chosen 必须 hard pass，尽量 travelmind soft pass。
- rejected 最好也 hard pass，但在一个明确 travelmind soft 维度失败。
...
- 比人工写 pair 更容易放大数据量。
- 可以直接围绕 travelmind soft 的失败类型采样。
...
第一版可以用 shaped reward，不建议只用 `travelmind_soft_pass` 这个 0/1 值，因为太稀疏。
...
下一轮 DPO 应继续围绕 `travelmind_soft_pass` 做高质量、单失败维度 pair。等 DPO 的收益开始变小，最值得尝试的 RL-style 方案是：
