...
| --- | --- | ---: | --- |
| standard | `training/data/travelmind/eval/records.jsonl` | 200 | `travelmind_standard200_realbudget_eval_` |
| hard | `training/data/travelmind/eval_hard/records.jsonl` | 300 | `travelmind_hard_realbudget_eval_` |

2026-05-11 高端 POI 召回增强后，主路径 `eval` / `eval_hard` 已重建 TravelMindContext；不要和 `training/data/travelmind/archive/eval_pre_high_end_context_20260511/` 下的旧 context 结果混比。
...
for path in [
    Path("training/data/travelmind/eval/records.jsonl"),
    Path("training/data/travelmind/eval_hard/records.jsonl"),
]:
    ids = []
    ...
    prefix = "travelmind_standard200_realbudget_eval_" if "eval_hard" not in str(path) else "travelmind_hard_realbudget_eval_"
...
.venv-training-py311/bin/python3 training/scripts/eval/eval_pipeline.py \
  ...
  --api-model travelmind-base-rebuild \
