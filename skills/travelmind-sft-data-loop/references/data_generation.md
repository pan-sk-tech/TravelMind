...
这份参考用于指导 TravelMind realbudget SFT 数据的生成阶段，包括请求分布 dry-run、TravelMindContext smoke、小批量 teacher 生成、`budget_supplement` 预算利用型补数和后续扩量。它只覆盖“怎么生成数据”和“生成后先检查什么”，不负责 budget fit audit、usability classification、LLaMAFactory 校验或训练判断。

生成阶段的目标不是一次性跑出大量 records，而是用低风险方式确认：请求分布合理、TravelMindContext 可构建、外部 API 并发可控、输出目录正确、teacher 生成没有明显系统性失败。


## 生成阶段的任务类型

- 请求分布 dry-run：只检查请求分布，不调用高德 API 或强模型。
- TravelMindContext smoke：小规模构建上下文，检查候选池、天气、酒店、餐饮、价格 hint 和预算策略。
...
- TravelMindContext smoke 会调用高德 API，但不调用强模型。
- teacher 生成会同时调用高德 API 和强模型 API，可能产生 token 成本。
- `--workers` 是样本级并发，会让多个样本同时构建 TravelMindContext 和调用 teacher 强模型。
...
- 确认高德缓存 TTL 或项目当前配置不会因为过期触发大面积重新请求；必要时先检查 `backend/app/travelmind_core/amap.py` 和 `.env` 里的缓存相关配置。
...
- TravelMindContext smoke：`--workers 1 --amap-qps-limit 3`
...
TravelMind SFT 原始生成 run 必须写入 `training/data/travelmind/sft/` 下的独立目录，不得直接把 records 或派生产物散写到 `training/data/travelmind/sft/` 根目录。
...
```text
training/data/travelmind/sft/<YYMMDD>_<run_slug>/
```

示例：

```text
training/data/travelmind/sft/260510_smoke20_main/
training/data/travelmind/sft/260510_validate100_main/
training/data/travelmind/sft/260510_main1000_gate1_thinking_off/
training/data/travelmind/sft/260510_smoke20_retry_api_limit/
training/data/travelmind/sft/260510_validate100_food_fix/
```

目录分工：

- `training/data/travelmind/sft/`：TravelMind SFT 数据根目录；原始生成 run 和后续清洗、分类、切分产物都必须放在可识别的子目录下。
- `training/data/travelmind/archive/`：仅用于留痕；不作为当前 TravelMind SFT run 的输入或输出目录。
...
- 确认输出目录位于 `training/data/travelmind/sft/` 下的独立子目录。
- 确认不会直接把文件写到 `training/data/travelmind/sft/` 根目录。
...
```bash
.venv-training-py311/bin/python3 training/scripts/travelmind/data/generate_sft_data.py \
  --count 100 \
...
- 如果 dry-run 分布不合理，先停下来调整请求生成逻辑，不进入 TravelMindContext smoke。
...
### TravelMindContext smoke 检查

用途：小规模构建 TravelMindContext，检查候选池、天气、酒店、餐饮、价格 hint、budget_fit_policy 和上下文压缩是否正常。不调用 TravelMind teacher 强模型，不写训练 records。
...
.venv-training-py311/bin/python3 training/scripts/travelmind/data/generate_sft_data.py \
  ...
  --amap-qps-limit 3 \
  --output-dir training/data/travelmind/sft/<YYMMDD>_mimo_realbudget_usage_validate100_w50
```
...
.venv-training-py311/bin/python3 training/scripts/travelmind/data/generate_sft_data.py \
  ...
  --teacher-model-provider mimo \
  --amap-qps-limit 3 \
  --output-dir training/data/travelmind/sft/<YYMMDD>_mimo_realbudget_usage_comfortable500_w50
```
...
.venv-training-py311/bin/python3 training/scripts/travelmind/data/generate_sft_data.py \
  ...
  --teacher-model-provider mimo \
  --amap-qps-limit 3 \
  --output-dir training/data/travelmind/sft/<YYMMDD>_mimo_realbudget_usage_premium200_w50
```
...
用途：生成少量 TravelMind realbudget teacher records，用于人工预览和最小验收。smoke20 的目标是发现系统性问题，不直接视为可训练数据定稿。

运行前确认：

- 已确认输出目录位于 `training/data/travelmind/sft/` 下的独立子目录。
- 已确认高德 API 和强模型 API 的 QPS、并发和成本边界。
- 已完成请求分布 dry-run 和 TravelMindContext smoke，且没有明显系统性问题。
- 生成 wrapper 只写本轮 run 的 records、errors、requests、summary 和 LLaMAFactory 文件，不合并历史数据。

示例命令：

```bash
.venv-training-py311/bin/python3 training/scripts/travelmind/data/run_sft_realistic_budget_generation.py \
  --count 20 \
  --target-successes 20 \
  --max-submissions 80 \
  --date-mode mixed \
  ...
  --output-dir training/data/travelmind/sft/<YYMMDD>_smoke20_main \
  --amap-qps-limit 3
```
...
- `--output-dir`：必须是 `training/data/travelmind/sft/` 下的独立 run 目录，不得直接写入 `training/data/travelmind/sft/` 根目录。
...
- 本轮输出目录位于 `training/data/travelmind/sft/` 下的独立子目录，并且不与 smoke20 混写。
...
.venv-training-py311/bin/python3 training/scripts/travelmind/data/run_sft_realistic_budget_generation.py \
  ...
  --workers 10 \
  --teacher-model-provider deepseek \
  --output-dir training/data/travelmind/sft/<YYMMDD>_validate100_main \
  --amap-qps-limit 3
```
...
.venv-training-py311/bin/python3 training/scripts/travelmind/data/run_sft_realistic_budget_generation.py \
  ...
  --workers 10 \
  --teacher-model-provider deepseek \
  --output-dir training/data/travelmind/sft/<YYMMDD>_main1000 \
  --amap-qps-limit 3
```
