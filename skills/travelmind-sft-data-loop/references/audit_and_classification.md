...
这份参考用于指导 TravelMind realbudget SFT records 的完整 records 审计和可用性分类。它通常在 smoke 数据已经完成样本预览、且没有明显系统性 teacher 质量问题之后使用。

它不负责生成数据，也不负责人工预览；它负责用项目脚本检查请求预算、teacher budget、候选池可达性，并把样本分成可用、可修、仅非预算可用或应丢弃等类别。

## 什么时候读取

- 用户要求审计一批 TravelMind realbudget SFT records。
...
使用脚本：`training/scripts/travelmind/audit/audit_sft_budget_fit.py`

用途：对一批 TravelMind realbudget SFT records 做第一轮系统审计。虽然脚本名包含 `budget_fit`，但在本 skill 中它代表整批 records 的基础审计：请求预算、teacher budget、候选池预算可达性、餐饮候选数量，以及按预算档位和 strictness 的问题分布。
...
```bash
.venv-training-py311/bin/python3 training/scripts/travelmind/audit/audit_sft_budget_fit.py \
...
使用脚本：`training/scripts/travelmind/audit/classify_sft_budget_usability.py`
...
```bash
.venv-training-py311/bin/python3 training/scripts/travelmind/audit/classify_sft_budget_usability.py \
