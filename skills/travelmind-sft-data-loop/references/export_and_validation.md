...
这份参考用于指导 TravelMind SFT run 做样本预览、导出训练候选子集，并完成最小格式校验。
...
- 用户要求把某轮 TravelMind SFT records 导出成训练候选数据。
...
```text
training/data/travelmind/sft/<YYMMDD>_<run_slug>/records.jsonl
training/outputs/eval/audits/<audit_root>/usability/record_classification.jsonl
```

导出结果应写到 `training/data/travelmind/sft/` 下新的、可识别的子目录；校验报告或命令输出记录应放到同一轮 `<audit_root>/export_validation/` 下。`<audit_root>` 应与审计和分类阶段一致，可以是 `<YYMMDD>_<run_slug>_records_audit/`，也可以是项目已经采用的 `<YYMMDD>_<run_slug>/`。

推荐目录：

```text
training/data/travelmind/sft/<YYMMDD>_<run_slug>_<category_slug>/
...
```text
training/data/travelmind/sft/260510_main1000_budget_clean/
training/data/travelmind/sft/260510_main1000_candidate_tight/
...
使用脚本：`training/scripts/travelmind/data/preview_sft_records.py`
...
```bash
.venv-training-py311/bin/python3 training/scripts/travelmind/data/preview_sft_records.py \
  --records training/data/travelmind/sft/<YYMMDD>_<run_slug>/records.jsonl \
  --errors training/data/travelmind/sft/<YYMMDD>_<run_slug>/errors.jsonl \
...
使用脚本：`training/scripts/travelmind/data/export_sft_budget_clean_subset.py`
...
```bash
.venv-training-py311/bin/python3 training/scripts/travelmind/data/export_sft_budget_clean_subset.py \
  --records training/data/travelmind/sft/<YYMMDD>_<run_slug>/records.jsonl \
  --classification training/outputs/eval/audits/<audit_root>/usability/record_classification.jsonl \
  --category usable_budget_clean \
  --output-dir training/data/travelmind/sft/<YYMMDD>_<run_slug>_budget_clean \
  --dataset-prefix travelmind_sft_<YYMMDD>_<run_slug>_budget_clean \
  --val-ratio 0.1
```
...
.venv-training-py311/bin/python3 training/scripts/validation/validate_trip_plan.py \
  --sft training/data/llamafactory/travelmind_sft_<YYMMDD>_<run_slug>_budget_clean_train.json

.venv-training-py311/bin/python3 training/scripts/validation/validate_trip_plan.py \
  --sft training/data/llamafactory/travelmind_sft_<YYMMDD>_<run_slug>_budget_clean_val.json
```
