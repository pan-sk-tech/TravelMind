---
name: travelmind-sft-data-loop
description: 当用户要为 travelmind-agent 生成一轮 TravelMind realbudget SFT 数据，并按项目最小验收流程判断这轮数据是否可以扩量、修复或作为训练候选时使用；覆盖 controlled/budget_supplement 请求分布检查、TravelMindContext smoke、小批量生成、预算审计、可用性分类、样本预览、导出校验和训练前 handoff。
---

# TravelMind SFT Data Loop

## 职责范围

本 skill 用于旅行助手项目的 TravelMind realbudget SFT 数据流程，关注对象是一轮数据 run，而不是整个模型训练项目。

它指导智能体按仓库已有脚本和 TravelMind 数据口径完成数据生成前检查、小批量生成、最小验收和下一步判断。生成一批数据后，不只看是否成功写出 records，还要检查预算、候选池、teacher 输出、样本可用性和导出格式。

本 skill 不替代仓库脚本，也不重新定义 TravelMind 数据口径。它只负责告诉智能体：当前场景该运行哪些已有脚本，运行前后要检查什么，哪些输出可以作为继续扩量、修复或进入训练候选的依据。
...
- 不负责 TravelMindContext 协议、TripPlan schema、后端 travelmind 逻辑或前端请求字段改造。
- 如果发现票价表、候选池或 TravelMindContext 问题阻塞本轮数据生成，只做定位和记录；需要结构性改造时，让用户确认。

## 硬规则

- TravelMind SFT 原始生成 run 必须写入 `training/data/travelmind/sft/` 下带日期和 run slug 的独立目录。
- 当前 skill 只处理当前 TravelMind SFT run；不默认做跨口径合并。
...
- 确认当前工作目录是 `travelmind-agent` 项目根目录。
...
- 明确输出目录，并确认原始生成 run 位于 `training/data/travelmind/sft/` 下带日期和 run slug 的独立目录。
...
2. TravelMindContext smoke：小规模构建上下文，检查高德候选、天气、酒店、餐饮、价格 hint 和 budget_fit_policy 是否正常。
...
- 需要检查请求分布、TravelMindContext smoke、生成 smoke20/100/1000 数据，或选择 `--workers`、输出目录、API 并发策略时，读取 `references/data_generation.md`。
