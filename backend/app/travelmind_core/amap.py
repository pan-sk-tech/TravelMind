...
TRAVELMIND_CONTEXT_PER_KEYWORD_LIMIT = int(os.getenv("TRAVELMIND_CONTEXT_PER_KEYWORD_LIMIT", "5"))
TRAVELMIND_CONTEXT_CACHE_DIR = Path(
    os.getenv("TRAVELMIND_CONTEXT_CACHE_DIR", "training/data/cache/travelmind_context")
)
TRAVELMIND_CONTEXT_CACHE_TTL_SECONDS = int(
    os.getenv("TRAVELMIND_CONTEXT_CACHE_TTL_SECONDS", str(60 * 24 * 60 * 60))
)
TRAVELMIND_CONTEXT_AMAP_CACHE_TTL_SECONDS = int(
    os.getenv("TRAVELMIND_CONTEXT_AMAP_CACHE_TTL_SECONDS", str(TRAVELMIND_CONTEXT_CACHE_TTL_SECONDS))
)
TRAVELMIND_CONTEXT_CLASSIC_CACHE_TTL_SECONDS = int(
    os.getenv("TRAVELMIND_CONTEXT_CLASSIC_CACHE_TTL_SECONDS", str(TRAVELMIND_CONTEXT_CACHE_TTL_SECONDS))
)
TRAVELMIND_CONTEXT_CACHE_VERBOSE = os.getenv("TRAVELMIND_CONTEXT_CACHE_VERBOSE", "0") == "1"
...
class AmapTravelMindClient:
    """面向 TravelMindContext 的高德查询客户端。"""

    def __init__(self, api_key: str, cache_dir: Path = TRAVELMIND_CONTEXT_CACHE_DIR):
    ...
    def get(self, path: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        cached = self._read_cache(
            cache_path,
            value_key="data",
            ttl_seconds=TRAVELMIND_CONTEXT_AMAP_CACHE_TTL_SECONDS,
        )
        if cached is not None:
            if TRAVELMIND_CONTEXT_CACHE_VERBOSE:
    ...
    def search_keywords(
        ...
        for keyword in keywords:
            ...
            rows.extend(normalized[:TRAVELMIND_CONTEXT_PER_KEYWORD_LIMIT])
    ...
    def search_classic_pois(self, city: str, keywords: List[str], limit: int) -> List[Dict[str, Any]]:
        ...
        cached = self._read_cache(
            cache_path,
            value_key="rows",
            ttl_seconds=TRAVELMIND_CONTEXT_CLASSIC_CACHE_TTL_SECONDS,
        )
        if cached is not None:
    ...
    def _resolve_cache_dir(self, path: Path) -> Path:
        ...
        if path.is_absolute():
            return path
        # backend/app/travelmind_core/amap.py -> project root
    ...
    def _read_cache(
        ...
        path: Path,
        value_key: str = "rows",
        ttl_seconds: int = TRAVELMIND_CONTEXT_CACHE_TTL_SECONDS,
    ) -> Optional[Any]:
        """读取未过期缓存。"""
        if not path.exists():
            return None
