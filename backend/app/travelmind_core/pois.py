"""TravelMind POI 关键词、规范化、过滤和排序逻辑。"""
...
def build_food_keyword_groups(request: TripRequest) -> List[Dict[str, Any]]:
    ...
    餐饮和景点不一样：TravelMind 每天都要安排午晚餐，只给一组全城
        ...
        rows.append(
            {
                ...
                # TravelMind不需要图片URL，保留数量即可，避免长URL把上下文撑大。
...
def is_noise_poi(row: Dict[str, Any], source_role: str) -> bool:
    ...
    if source_role == "food":
        # 餐饮候选优先只保留真正餐厅；水产市场/批发市场给TravelMind意义不大。
