...
def validate_trip_plan_shape(
    ...
    if travelmind_context:
        warn_plan_grounding(trip_plan, travelmind_context)


def warn_plan_grounding(trip_plan: TripPlan, travelmind_context: Dict[str, Any]) -> None:
    ...
    if ungrounded_attractions:
        ...
        print(f"⚠️  TravelMind输出中有未命中工具候选的景点: {preview}")
    if ungrounded_hotels:
        preview = ", ".join(ungrounded_hotels[:3])
        print(f"⚠️  TravelMind输出中有未命中工具候选的酒店: {preview}")
    ...
    if ungrounded_meals:
        ...
        print(f"⚠️  TravelMind输出中有未命中餐饮候选的餐饮: {preview}")


def enrich_trip_plan_poi_details(trip_plan: TripPlan, travelmind_context: Dict[str, Any]) -> None:
    """用工具候选回填模型容易漏掉的 POI 地址和坐标。

    TravelMind 有时只复制了餐厅名和价格，没有把 food_pois 中的 address/location
