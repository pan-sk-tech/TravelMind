...
def compact_for_travelmind(travelmind_context: Dict[str, Any]) -> Dict[str, Any]:
    """把原始工具快照压缩成模型真正需要看的 TravelMind 输入。"""
    snapshot = travelmind_context.get("tool_snapshot", {})
    return {
        "version": travelmind_context.get("version", "travelmind_context"),
        "request": travelmind_context.get("request", {}),
        "party": travelmind_context.get("party", {}),
        "budget_constraint": travelmind_context.get("budget_constraint", {}),
        "preference_profile": travelmind_context.get("preference_profile", {}),
        "lodging_policy": travelmind_context.get("lodging_policy", {}),
        "pricing_policy": travelmind_context.get("pricing_policy", {}),
        "route_policy": travelmind_context.get("route_policy", {}),
        "tool_snapshot": {
        ...
        "travelmind_constraints": travelmind_context.get("travelmind_constraints", {}),
    }
...
def compact_food_query_groups(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """保留餐饮召回分桶，方便 TravelMind 理解 food_pois 覆盖面。"""
    results = []
    for item in rows:
        compact = {
...
def compact_type(value: Any) -> str:
    """高德 type 往往很长，只取最能给 TravelMind 提示用途的部分。"""
    text = str(value or "")
    if "|" in text:
        text = text.split("|", 1)[0]
