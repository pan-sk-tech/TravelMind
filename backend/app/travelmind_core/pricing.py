"""TravelMind 价格 hint 规则。

这里的价格不是实时交易价，而是给 TravelMind 和训练数据使用的稳定预算口径。
"""
...
from .output import normalize_poi_name as normalize_travelmind_poi_name
...
ATTRACTION_PRICE_TABLE_PATH = Path(
    os.getenv("TRAVELMIND_ATTRACTION_PRICE_TABLE_PATH", str(DEFAULT_ATTRACTION_PRICE_TABLE_PATH))
)
...
def normalize_poi_name(value: Any) -> str:
    """景点名归一化，用于票价表匹配。"""
    return normalize_travelmind_poi_name(str(value or ""))
