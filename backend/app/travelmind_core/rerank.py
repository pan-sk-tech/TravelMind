...
def rerank_trip_plan_candidates(
    ...
    request: TripRequest,
    travelmind_context: Dict[str, Any] | None,
) -> List[RerankCandidate]:
    """为候选 TripPlan 打分并按分数降序返回。"""
    scored: List[RerankCandidate] = []
    for attempt, trip_plan in candidates:
        metrics = score_trip_plan_candidate(trip_plan, request, travelmind_context or {})
...
def score_trip_plan_candidate(
    trip_plan: TripPlan,
    request: TripRequest,
    travelmind_context: Dict[str, Any],
) -> Dict[str, Any]:
    """计算一个 TripPlan 候选的 rerank 指标。"""
    snapshot = (travelmind_context.get("tool_snapshot") or {}) if isinstance(travelmind_context, dict) else {}
...
def budget_fit_details(total: int, request: TripRequest, travelmind_context: Dict[str, Any]) -> Dict[str, Any]:
    """评估重算预算是否贴合预算策略。"""
    amount = _safe_int(request.budget_constraint.amount)
    strictness = str(request.budget_constraint.strictness or "none").strip().lower()
    policy = ((travelmind_context.get("travelmind_constraints") or {}).get("budget_fit_policy") or {})
