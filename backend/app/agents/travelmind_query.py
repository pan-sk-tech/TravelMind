"""TravelMind Agent input construction and output token budget."""
...
TRAVELMIND_OUTPUT_BASE_TOKENS = int(os.getenv("TRAVELMIND_OUTPUT_BASE_TOKENS", "3000"))
TRAVELMIND_OUTPUT_TOKENS_PER_DAY = int(os.getenv("TRAVELMIND_OUTPUT_TOKENS_PER_DAY", "1800"))
TRAVELMIND_MAX_OUTPUT_TOKENS_CAP = int(os.getenv("TRAVELMIND_MAX_OUTPUT_TOKENS_CAP", "16000"))


def build_travelmind_query(travelmind_context_builder: Any, request: TripRequest, travelmind_context: Dict[str, Any]) -> str:
    """Build the final TravelMind Agent user input."""
    prompt_context = travelmind_context_builder.compact_for_travelmind(travelmind_context)
    raw_context_json = json.dumps(travelmind_context, ensure_ascii=False, separators=(",", ":"))
    context_json = json.dumps(prompt_context, ensure_ascii=False, separators=(",", ":"))
    print(f"TravelMindContext压缩: {len(raw_context_json)} -> {len(context_json)} 字符", flush=True)
    query = f"""请根据下面的 TravelMindContext JSON 生成{request.city}的{request.travel_days}天旅行计划。
...
7. 价格字段必须复制TravelMindContext里的hint：hotel.estimated_cost=estimated_cost_hint，表示单间每晚房价；attraction.ticket_price=ticket_price_hint，表示成人单人票价；meal.estimated_cost=meal_cost_hint，表示单人单餐费用。价格复制优先级高于预算贴合；预算只能通过选择更高价的真实候选来贴近，绝不能为了凑预算修改任何候选单价。预算汇总必须考虑party.total、rooms=ceil(party.total/2)和实际住宿晚数。
...
12. 如果 TravelMindContext.travelmind_constraints.budget_fit_policy 存在，budget.total 应尽量落在 target_min_total 和 target_max_total 之间；不要只做最低价方案。若真实候选价格不足以完全用满预算，选择最接近预算的真实候选组合并在overall_suggestions中说明，不能抬高免费/低价景点门票，不能复用同一家餐厅凑预算。
"""
...
def travelmind_max_output_tokens(request: TripRequest) -> int:
    """按行程天数动态估算 TravelMind 输出上限。"""
    return min(
        TRAVELMIND_OUTPUT_BASE_TOKENS + request.travel_days * TRAVELMIND_OUTPUT_TOKENS_PER_DAY,
        TRAVELMIND_MAX_OUTPUT_TOKENS_CAP,
    )
